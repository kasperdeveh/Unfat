...
- K-review follow-ups + security sweep: drie `with check`-clauses toegevoegd aan update-policies om manipulatie via directe API-calls te blokkeren. Belangrijkste vondst: `profiles_update_own` had geen `with check` op de `role`-kolom (toegevoegd in J-A) — een normale user kon zichzelf via een PostgREST PATCH naar `admin` promoten. Tijdens deze sessie gefixt vóór misbruik. Ook: `dishes_update_by_editor` en `products_update_user_by_editor` locken nu `created_by` zodat een editor geen ownership kan kapen. Migration `20260504174110_rls_with_check_hardening.sql`. Sweep verder schoon bevonden: SQL-injectie via SDK-parametrisering afgedekt; alle SECURITY DEFINER RPCs hebben `set search_path = public` + correcte auth-checks; triggers draaien als invoker met RLS van toepassing
